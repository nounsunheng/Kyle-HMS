    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form method="GET" action="{{ route('doctor.appointments.index') }}"
                ...
                <div class="flex items-end">
                    <button type="submit"
                        class="w-full bg-secondary-600 hover:bg-secondary-700 text-white font-semibold py-2 px-4 rounded-md transition duration-150">
                        Apply Filters
                    </button>
                </div>
            </form>
        ...
        @else
            <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                ...
                <p class="mt-2 text-sm text-gray-500">Try adjusting your filters or create a schedule to start accepting
                    appointments</p>
                @if (!request()->hasAny(['status', 'date']))
                    <a href="{{ route('doctor.schedule.create') }}"
                        class="mt-4 inline-block text-secondary-600 hover:text-secondary-700">
                        Create Schedule
                    </a>
                @else
